...
export const GroupBuyAdminView: React.FC = () => {
  ...
  const mapBackendStatus = (status: string): '모집중' | '성공' | '무산' | '신고됨' => {
    if (status === 'RECRUITING') return '모집중';
    if (status === 'SUCCESS' || status === 'CLOSED') return '성공';
    if (status === 'FAILED') return '무산';
    return '신고됨';
  };

  const mapFrontendStatusToBackend = (status: '모집중' | '성공' | '무산' | '신고됨'): string => {
    if (status === '모집중') return 'RECRUITING';
    if (status === '성공') return 'SUCCESS';
    if (status === '무산') return 'FAILED';
    return 'BLIND';
  };
  ...
  const loadDashboardStats = useCallback(async () => {
    try {
      ...
      if (res.success && res.data) {
        const d = res.data;
        setDashboardStats({
          openedToday: d.todayCreatedCount,
          totalParticipants: d.activeParticipantsCount,
          recruiting: d.recruitingCount,
          success: d.successCount,
          failed: d.failedCount,
          reported: 0,
          recruitingRate: Math.round(d.recruitingRatio),
          successRate: Math.round(d.successRatio),
          failedRate: Math.round(d.failedRatio),
          reportedRate: 0,
          total: d.recruitingCount + d.successCount + d.failedCount
        });
      }
    } catch (err) {
  ...
  const loadPosts = useCallback(async () => {
    try {
      const res = await fetchWithAuth('/api/v1/admin/group-purchases?size=100');
      if (res.success && res.data && res.data.content) {
        const mapped = res.data.content.map((gp: any) => ({
          id: gp.id,
          category: gp.categoryName || '미지정',
          title: gp.title,
          owner: gp.creatorUsername || '탈퇴한 사용자',
          price: gp.price,
          currentCount: gp.currentParticipants,
          targetCount: gp.maxParticipants,
          createdAt: gp.createdAt ? gp.createdAt.replace('T', ' ').substring(0, 16) : '',
          status: mapBackendStatus(gp.status)
        }));
        setPosts(mapped);
      }
    } catch (err) {
  ...
  const loadCategories = useCallback(async () => {
    try {
      const res = await fetchWithAuth('/api/v1/group-purchase-categories');
      if (res.success && res.data) {
  ...
  const loadReports = useCallback(async () => {
    try {
      const res = await fetchWithAuth('/api/v1/admin/reports?size=100');
      if (res.success && res.data && res.data.content) {
        const mapped = res.data.content.map((rep: any) => {
          let statusText: '대기중' | '경고처리' | '삭제처리' | '반려됨' = '대기중';
          if (rep.isProcessed) {
            if (rep.processResult === 'WARNED') statusText = '경고처리';
            else if (rep.processResult === 'DELETED') statusText = '삭제처리';
            else statusText = '반려됨';
          }
          return {
            id: rep.id,
            type: rep.targetType === 'GROUP_PURCHASE' ? '글' : '댓글',
            targetId: rep.targetId,
            content: `${rep.targetType === 'GROUP_PURCHASE' ? '공동구매 글' : '댓글'} #${rep.targetId}`,
            reporter: `사용자 #${rep.reporterId}`,
            reason: rep.reason,
            status: statusText,
            createdAt: rep.createdAt ? rep.createdAt.replace('T', ' ').substring(0, 16) : ''
          };
        });
        setReports(mapped);
      }
    ...
    try {
      ...
      if (categories.length === 0) {
        ...
        if (catRes.success && catRes.data) {
          catList = catRes.data.map((cat: any) => ({
            id: cat.id,
            name: cat.name,
      ...
      if (res.success && res.data) {
        const mapped = res.data.map((prod: any) => ({
          ...
          createdAt: prod.createdAt ? prod.createdAt.split('T')[0] : ''
        }));
        setProducts(mapped);
      }
    } catch (err) {
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      try {
        ...
        setError('데이터를 가져오는 중 실패했습니다. 권한을 확인해 주세요.');
      } finally {
        setLoading(false);
      }
    };
    loadAll();
  }, [activeAdminSubTab, loadDashboardStats, loadPosts, loadCategories, loadReports, loadProducts]);

  // --- Handlers for Post Management ---
  const handleUpdatePostStatus = async (postId: number, newStatus: '모집중' | '성공' | '무산' | '신고됨') => {
    try {
      const backendStatus = mapFrontendStatusToBackend(newStatus);
      const res = await fetchWithAuth(`/api/v1/admin/group-purchases/${postId}/status?status=${backendStatus}`, {
        method: 'PUT'
      });
      if (res.success) {
        loadPosts();
      ...
      alert('상태 조정에 실패했습니다: ' + (err instanceof Error ? err.message : err));
    }
  };
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    try {
      if (categoryModalMode === 'add') {
        ...
        const res = await fetchWithAuth(`/api/v1/group-purchase-categories/${selectedCategoryId}`, {
          method: 'PUT',
          body: JSON.stringify({
            name: categoryForm.name,
      ...
      alert('카테고리 저장에 실패했습니다: ' + (err instanceof Error ? err.message : err));
    }
  };
  ...
  const handleResolveReport = async (reportId: number, _targetId: number, _type: '글' | '댓글', action: '경고' | '삭제' | '반려') => {
    try {
      let backendAction = 'DISMISSED';
      if (action === '경고') backendAction = 'WARNED';
      else if (action === '삭제') backendAction = 'DELETED';

      const res = await fetchWithAuth(`/api/v1/admin/reports/${reportId}/process`, {
        method: 'POST',
        body: JSON.stringify({
          action: backendAction,
          reason: `${action} 처리를 수행했습니다.`
        })
      });
      if (res.success) {
        loadReports();
      ...
      alert('신고 처리에 실패했습니다: ' + (err instanceof Error ? err.message : err));
    }
  };
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      const matchedCatId = categories.find(c => c.name === productForm.category)?.id;
      if (!matchedCatId) {
      ...
      if (productModalMode === 'add') {
        ...
        const res = await fetchWithAuth(`/api/v1/products/${selectedProductId}`, {
          method: 'PUT',
          body: JSON.stringify(payload)
        });
        if (res.success) {
      ...
      alert('상품 저장에 실패했습니다: ' + (err instanceof Error ? err.message : err));
    }
  };
